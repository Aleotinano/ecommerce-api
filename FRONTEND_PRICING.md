...
Cada variante **puede** tener su propio precio. Si no lo tiene (`price: null`),
hereda el precio del producto. Esto habilita **productos unitarios**: un producto
con `price` propio y variantes sin precio (o, conceptualmente, "una sola cosa").
...
| Sin precio en ningún lado | `null` | `null` | **null** → no comprable |
...
- **Precio efectivo `null`** (producto y variante sin precio) → mostrar `"Consultar"`
  y deshabilitar el botón de compra. No mandes ese ítem al carrito.
...
El campo `price` del producto es **opcional** y **nullable**. Las variantes también
aceptan `price` opcional/nullable.
...
- `price` (producto y variante) debe ser **> 0** si se envía; podés mandar `null`
  para "sin precio". `stock` sigue siendo obligatorio en cada variante.
- Si omitís `price` al crear, queda en `null`.
...
- [ ] Deshabilitar compra cuando el precio efectivo es `null`.
- [ ] Carrito: usar `variant.price` (efectivo) para subtotales.
- [ ] Admin: formularios de producto y variante con `price` opcional (permitir vaciarlo → `null`).
