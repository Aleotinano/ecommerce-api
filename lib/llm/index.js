...
export const generateCopy = async ({ product, angle, config }) => {
  const provider = PROVIDERS[DEFAULTS.LLM.PROVIDER] ?? geminiProvider;
  const { system, user } = buildPrompt({ product, angle, config });
  ...
  try {
    ...
    if (!parsed) {
      ...
      return { ...buildFallbackCopy({ product, angle, config }), model: null };
    }
