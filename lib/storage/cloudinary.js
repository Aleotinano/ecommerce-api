...
function folderFor(tenantId, entity) {
  ...
  // POR TENANT. Hoy todos los tenants comparten una cuenta de Cloudinary, pero el
  // plan es que cada cliente de producción tenga la suya: con la carpeta ya
  // aislada, esa migración es mover una carpeta en vez de filtrar fila por fila.
 ...
 */
export async function deleteFile({ publicId, resourceType, deliveryType }) {
  if (!publicId) return;
